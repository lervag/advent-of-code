  ...
  while (!work.isEmpty) {
    ...
    directions
      ...
      .flatMap { (n, m) =>
        ...
        val i = r + n
        val j = c + m
        if (i >= 0 && i < heat.size && j >= 0 && j < heat.size && repeats < 3)
          Some(State((i, j), current.distance + heat(i)(j), (n, m), repeats))
        else
          None
      }
      .filter { s => s.distance < distances((s.pos, s.direction)) }
      .foreach { state =>
        distances((state.pos, state.direction)) = state.distance
        pathMap(state) = current
        work.enqueue(state)
      }
  }

  val endStates = pathMap.keys.filter(_.pos == destination)
  println(endStates)
  val endState = endStates.last
